#ifndef RCPPSVAHCLUST_HCLUST_H
#define RCPPSVAHCLUST_HCLUST_H
...
void populate_Rhclust(const Clusters& clusters, Hclust& hclust) {
  ...
  if (clusters.size() != (2 * hclust.agglomerations() + 1)) {
    std::invalid_argument("RcppsvaHClust clusters and hclust object inconsistently sized");
  }
...
template <typename T> SEXP wrap( const RcppsvaHClust::ClusterVector<T>& clusters ) {
  RcppsvaHClust::Hclust hclust(clusters.initial_clusters());
  RcppsvaHClust::populate_Rhclust(clusters, hclust);
